...
let parse = (src, anchors) => {
	 ...
	 */
	const prop = (neg, p) => {
		PROP.test(p) || bad(); // oxlint-disable-line no-unused-expressions
	...
	const esc = () => {
		...
		if ((c === 'p' || c === 'P') && s[i] === '{') {
			...
			s[i++] === '}' || bad(); // oxlint-disable-line no-unused-expressions
			return prop(c === 'P', p);
		}
		'()*+-.?[\\]^nrt{|}'.includes(c) || bad(); // oxlint-disable-line no-unused-expressions
		const v = c === 'n' ? '\n' : c === 'r' ? '\r' : c === 't' ? '\t' : c;
		return { p: x => x === v, c: v };
	};
	/** @returns {Node} */
	const cls = () => {
		...
		const one = () => {
			const c = s[i++];
			if (c === '\\') return esc();
			(c != null && c !== '[' && c !== ']' && c !== '-') || bad(); // oxlint-disable-line no-unused-expressions
			return { p: x => x === c, c };
		};
		...
		while (i < s.length && s[i] !== ']') {
			...
			if (s[i] === '-' && s[i + 1] !== ']') {
				...
				(a.c != null && b.c != null && /** @type {number} */ (a.c.codePointAt(0)) <= /** @type {number} */ (b.c.codePointAt(0))) || bad(); // oxlint-disable-line no-unused-expressions
		...
		(ps.length && s[i++] === ']') || bad(); // oxlint-disable-line no-unused-expressions
		return [1, (c, spend) => {
			let yes = false;
			for (const p of ps) {
				spend();
	...
	const number = () => {
		...
		while (i < s.length && s[i] >= '0' && s[i] <= '9') {
			++d <= 6 || cap('TREFFER_MAX_QUANTIFIER_DIGITS', 6, d); // oxlint-disable-line no-unused-expressions
			n = n * 10 + +s[i++];
			n <= REPEAT || cap('TREFFER_MAX_REPETITIONS', REPEAT, n); // oxlint-disable-line no-unused-expressions
		}
		d || bad(); // oxlint-disable-line no-unused-expressions
		return n;
	};
	/** @type {() => Node} */
	let alt;
	...
	const atom = () => {
		...
		(c != null && !'()[]|*+?{}'.includes(c)) || bad(); // oxlint-disable-line no-unused-expressions
		return [1, x => x === c];
	};
	/** @returns {Node} */
	const piece = () => {
	...
	const out = alt();
	i === s.length || bad(); // oxlint-disable-line no-unused-expressions
	return out;
};
...
let build = (pattern, anchors) => {
	...
	const add = (t, x = -1, y = -1, v) => {
		st.length < MAX || cap('TREFFER_MAX_NFA_STATES', MAX, st.length + 1); // oxlint-disable-line no-unused-expressions
...
let run = (nfa, str, full) => {
	...
	const add = (set, root, pos) => {
		...
		while (todo.length) {
			...
			if (q[0] === 2) { (q[3] ? pos === len : pos === 0) && todo.push(q[1]); continue } // oxlint-disable-line no-unused-expressions
			set.add(j);
		}
	};
