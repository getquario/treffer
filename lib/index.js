...
let build = (pattern, anchors) => {
  const ast = parse(pattern, anchors),
    states = /** @type {State[]} */ ([]);
  /**
   * @param {number} t
   * @param {number} [x]
  ...
  const visit = (n) => {
    if (!n[0]) return empty();
    if (n[0] === 1) {
      const j = add(1, -1, -1, n[1]);
      return { s: j, o: [[j, 1]] };
    }
    if (n[0] === 2) {
      const j = add(2, -1, -1, n[1]);
      return { s: j, o: [[j, 1]] };
    }
    if (n[0] === 3) {
      let a = visit(n[1][0]);
...
let run = (nfa, str, full) => {
  ...
  typeof str === "string" ||
    (() => {
      throw fault(TypeError, "Subject must be a string");
    })();
  scalarCount(
    str,
    STEPS,
    () => {
      throw fault(TypeError, "Subject must contain Unicode scalar values");
    },
    "TREFFER_MAX_SUBJECT_SCALARS",
  );
  const { states, start, end } = nfa,
    len = str.length;
  let cur = /** @type {Set<number>} */ (new Set()),
    steps = 0;
  const spend = () => {
    // oxlint-disable-next-line no-unused-expressions
  ...
  const add = (set, root, pos) => {
    ...
    while (todo.length) {
      ...
      if (state[0] === 2) {
        // oxlint-disable-next-line no-unused-expressions
        (state[3] ? pos === len : pos === 0) && todo.push(state[1]);
        continue;
      }
      set.add(j);
    }
  ...
  for (let pos = 0; pos <= len;) {
    ...
    const n = /** @type {number} */ (str.codePointAt(pos)),
    ...
    for (const j of cur) {
      ...
      if (
        state[0] === 1 &&
        /** @type {(x: string, spend: () => void) => boolean} */ (state[3])(c, spend)
      )
        add(next, state[1], nextPos);
    }
    cur = next;
    pos = nextPos;
  }
...
export let compile = (pattern, options) => {
  ...
  typeof pattern === "string" ||
    (() => {
      throw fault(TypeError, "Pattern must be a string");
    })();
  if (options != null && typeof options !== "object")
    throw fault(TypeError, "Options must be an object");
  const anchors = options?.anchors ?? false;
  // oxlint-disable-next-line no-unused-expressions
  typeof anchors === "boolean" ||
    (() => {
      throw fault(TypeError, "anchors must be a boolean");
    })();
