 ...
 * `span` replaces it, for one whose text reached the pattern through a decode
 * and has no linear offset to shift.
 *
...
/**
 * The one syntax fault this module throws, from the UTF-16 offsets a span
 * ships in. `bad` converts for it; the lone-surrogate check already holds
 * string offsets and calls it directly.
 *
 ...
 * `TREFFER_MAX_*` half of `TrefferErrorCode`: the code is the contract a host
 * branches on, and this is the half a human reads off a stack trace.
 *
 ...
 * as `actual`; the pattern-length pre-check is the exception and calls
 * `capped` itself, because it rejects before a scalar count exists to report.
 *
 * @type {(value: number, limit: number, code: TrefferErrorCode) => boolean}
 */
const within = (value, limit, code) => value <= limit || budget(code, limit, value);

/**
 * Every exhausted budget goes through here, for the name its message needs.
 * The code is checked by the store, not by this signature.
 *
 * @type {(code: TrefferErrorCode, limit: number, actual?: number) => never}
 */
const budget = (code, limit, actual) => capped(diags, BUDGETS[code], code, limit, actual);
...
let esc = () => {
  ...
  // Only three escapes stand for a different character, and the two strings
  // below spell them in the same order: the letter the pattern used, then what
  // it means. Any other letter the whitelist allowed is not in the first, so it
  // indexes past the end of the second and the escape stands for itself. Either
  // way it is a literal, so it is built where every other literal is: `solo`
  // has already been told which characters are unavailable, and none are here.
...
/**
 * An atom and the quantifier following it, if any. The three one-character
 * quantifiers carry their bounds in the character itself — only `+` has a lower
 * bound, only `?` has an upper one — so they are read off it arithmetically,
 * without spending the branches a lookup table would cost here. `{` reads its
 * own bounds.
 *
 * @returns {Node}
 */
...
let parse = (src, wantAnchors) => {
  // oxlint-disable-next-line no-unused-expressions
  src.length <= MAX * 2 || budget("TREFFER_MAX_PATTERN_SCALARS", MAX);
...
visit = (n) =>
  n[0] < 3
    ...
    : n[0] === 3
      ...
      : n[0] === 4
        ? fold(/** @type {any} */ (n)[1], (a, b) => ({
            s: addState(0, a.s, b.s),
            o: a.o.concat(b.o),
          }))
        : quantified(n);
