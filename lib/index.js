...
/**
 * @type {(code: TrefferErrorCode, own: Record<string, number>) => never}
 */
const cap = (code, own) => fault(RangeError, CAPPED, code, own);
/**
 ...
 */
const within = (value, limit, code) => value <= limit || cap(code, { limit, actual: value });
/** @type {() => never} */
...
/** @returns {Node} */
let branch = () => list(3, [], () => stillBranch() && piece());
...
let parse = (src, wantAnchors) => {
  // oxlint-disable-next-line no-unused-expressions
  src.length <= MAX * 2 || cap("TREFFER_MAX_PATTERN_SCALARS", { limit: MAX });
  ...
  scalarCount(src, MAX, (j) => badAt(j, j + 1), "TREFFER_MAX_PATTERN_SCALARS");
...
let enqueue = (set, j, at, todo) => {
  ...
  // oxlint-disable-next-line no-unused-expressions
  else if (state[0] === 2) atAnchor(state[3], at) && todo.push(state[1]);
  else set.add(j);
};
