...
/**
 * One compiled character predicate, `[test, literal]`. The literal is present
 * only where the predicate came from a single character — class range
 * endpoints need it.
 *
 * @internal
 * @typedef {[(x: string) => boolean, string?]} Matcher
 */
...
/** A built machine, `[states, start, end]`. @internal @typedef {[State[], number, number]} Nfa */
...
/**
 * What a budget is called in the message it throws: the `TREFFER_MAX_` prefix
 * dropped and the rest spelled as prose. `NFA` is the one initialism among the
 * codes, so it is the one word lowercasing gets wrong.
 *
 * @type {(code: TrefferErrorCode) => string}
 */
const budgetName = (code) => code.slice(12).toLowerCase().replace(/_/g, " ").replace("nfa", "NFA");
...
const within = (value, limit, code) =>
  value <= limit || capped(diags, budgetName(code), code, limit, value);
...
let esc = () => {
  ...
  if ((char === "p" || char === "P") && chars[pos] === "{") return unicodeProp(char);
  // oxlint-disable-next-line no-unused-expressions
...
let rangePred = (preds, first) => {
  ...
  // A statement-level `|| bad()` is not a control-flow assertion, so the casts
  // restate what the guard below proves. The explicit `0` is `codePointAt`'s
  // default, which its signature requires.
  const codePoint = (/** @type {Matcher} */ m) =>
    /** @type {number} */ (/** @type {string} */ (m[1]).codePointAt(0));
  // oxlint-disable-next-line no-unused-expressions
  (pair.every((m) => m[1] != null) && codePoint(pair[0]) <= codePoint(pair[1])) || bad();
  const [low, high] = pair.map(codePoint);
  preds.push((x) => {
...
let classItem = (preds) => {
  ...
  const first = one();
  if (chars[pos] === "-" && chars[pos + 1] !== "]") rangePred(preds, first);
  else preds.push(first[0]);
};
...
let brace = () => {
  const from = pos++;
  const lo = number();
  const hi = chars[pos] !== "," ? lo : (pos++, chars[pos] === "}" ? -1 : number());
  expect("}");
...
let parse = (src, wantAnchors) => {
  // oxlint-disable-next-line no-unused-expressions
  src.length <= MAX * 2 ||
    capped(diags, budgetName("TREFFER_MAX_PATTERN_SCALARS"), "TREFFER_MAX_PATTERN_SCALARS", MAX);
...
let scan = (str, cur, nfa, full) => {
  ...
  for (;;) {
    // oxlint-disable-next-line no-unused-expressions
    full || add(cur, nfa[1], at);
    if (cur.has(nfa[2]) && (!full || at === len)) return true;
...
let run = (nfa, str, full) => {
  ...
  [states, len, steps] = [nfa[0], str.length, 0];
  const cur = /** @type {Set<number>} */ (new Set());
  if (full) add(cur, nfa[1], 0);
...
export let compile = (pattern, options) => {
  ...
  const wantAnchors = options?.anchors ?? false;
  // oxlint-disable-next-line no-unused-expressions
  typeof wantAnchors === "boolean" || mint(diags, TypeError, "anchors must be a boolean");
  const ast = parse(pattern, wantAnchors);
  states = /** @type {State[]} */ ([]);
  const f = visit(ast),
    end = addState(3);
  patch(f[1], end);
  const nfa = /** @type {Nfa} */ ([states, f[0], end]);
